<template>
  <el-dialog
    ...
    :title="mode === 'create' ? '新增支出类别' : '编辑支出类别'"
    width="500px"
    @close="handleClose"
  >
    <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
      <el-form-item label="分类" prop="fenlei">
        <el-input v-model="form.fenlei" placeholder="请输入分类（一级分类）" />
      </el-form-item>
...
import { createZhichuLeibie, updateZhichuLeibie, type ZhichuLeibie } from '@/api/modules/finance-settings'

const props = defineProps<{
  modelValue: boolean
  leibie: ZhichuLeibie | null
  mode: 'create' | 'edit'
}>()
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      if (props.mode === 'create') {
        await createZhichuLeibie(form.value)
        ElMessage.success('创建成功')
      } else {
        await updateZhichuLeibie(form.value.id!, form.value)
