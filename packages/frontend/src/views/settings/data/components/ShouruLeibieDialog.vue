<template>
  <el-dialog
    ...
    :title="mode === 'create' ? '新增收入类别' : '编辑收入类别'"
    width="500px"
    @close="handleClose"
  >
...
import { createShouruLeibie, updateShouruLeibie, type ShouruLeibie } from '@/api/modules/finance-settings'

const props = defineProps<{
  modelValue: boolean
  leibie: ShouruLeibie | null
  mode: 'create' | 'edit'
}>()
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      if (props.mode === 'create') {
        await createShouruLeibie(form.value)
        ElMessage.success('创建成功')
      } else {
        await updateShouruLeibie(form.value.id!, form.value)
