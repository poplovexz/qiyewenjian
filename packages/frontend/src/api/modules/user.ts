...
export const userApi = {
  ...
  assignRoles(userId: string, roleIds: string[]): Promise<boolean> {
    return http.post(`/api/v1/users/${userId}/roles`, { jiaose_ids: roleIds })
  },
  ...
  getUserPermissions(userId: string): Promise<string[]> {
    return http.get(`/api/v1/users/${userId}/permissions`)
  }
}
...
export const roleApi = {
  ...
  updateRole(id: string, data: any): Promise<Role> {
    return http.put(`/api/v1/roles/${id}`, data)
  },
  ...
  assignPermissions(roleId: string, permissionIds: string[]): Promise<boolean> {
    return http.post(`/api/v1/roles/${roleId}/permissions`, { quanxian_ids: permissionIds })
  }
}
